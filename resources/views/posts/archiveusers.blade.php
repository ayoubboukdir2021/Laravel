                    ...
                    <div class="card" style="margin-bottom: 15px;">
                        <div class="card-header bg-secondary"> 
                            ...
                            <div style="text-align:right;">
                                <form style="display:inline;" action="{{ route('restoreuser',['id' => $user->id]) }}" method="POST" class="form" >
                                    @csrf
                                    @method('PATCH')
                                        <input type="submit" class="btn btn-success btn-sm " value="restore"/></i>
                                </form>

                                <form style="display:inline;" method="POST" action="{{ route('forcedeleteuser',['id'=>$user->id]) }}">
                                    @csrf
                                    @method('DELETE')
